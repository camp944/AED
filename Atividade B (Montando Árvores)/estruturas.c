...
{
	int Valores_Iniciais[14]={7,9,10,12,22,31,40,55,61,74,80,85,91,98},contador;
	record r;
	
	for(contador=0;contador<14;contador++)
	{
		r.key=Valores_Iniciais[contador];
		Insere(p,r);
	}	
...
void InsereFix(Pagina **p, Pagina **pr, record r, record *rr, int *Overflow)
{
	...
	{ 
		printf("[ERROR]: O VALOR JA EXISTE\n"); 
		*Overflow = FALSE;
		return;
	}
...
void Imprime(Pagina **p, int lvl)
{
	...
	for (contador=0;contador<(*p)->n;contador++){printf("%d ",(*p)->r[contador].key);}
